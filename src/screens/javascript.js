...
export const javascript = ({ navigation }) => {
    let state = useSelector(state => state)
    let total = state.jsquestions.length
    ...
    const check = () => {
        let correctval = state.jsquestions[quesno].correctAns
        if (selectedvalue == correctval) {
            setscore(score + permarks)
        }

        if (quesno + 1 != state.jsquestions.length) {
            next()
        } else {
            setresult(true)
        }
            ...
            <ImageBackground style={styles.background} resizeMode="cover" source={{ uri: "https://i.pinimg.com/originals/d6/cb/e0/d6cbe01ac6372a505d6e86a31a1e5e23.png" }}>
                {result ?
                    ...
                    <View style={styles.background}>
                        ...
                        <View>
                            <Text style={styles.question}>{state.jsquestions[quesno].question}</Text>
                        </View>
                        {state.jsquestions[quesno].options.map((e, i) => {
                            let color = ""
                            let background = ""
                            if (selectedvalue != e) {
                                color = "#c40a0d"
